...
restaurants_and_reviews = pd.read_csv("../data/restaurants_and_reviews.csv")
...
# businesses_and_reviews.to_csv('restaurants_and_reviews.csv')
# businesses_and_reviews.head()

restaurants_and_reviews['env_terms'] = restaurants_and_reviews['text'].apply(lambda x: get_env_terms(x))
restaurants_and_reviews['env_term_counts'] = restaurants_and_reviews[['text', 'env_terms']].apply(
    lambda x: get_env_term_counts(x), axis=1)
restaurants_and_reviews.sort_values(['env_term_counts'], ascending=False, inplace=True)

restaurants_and_reviews['env_terms_percent_of_overall_words'] = (restaurants_and_reviews['env_term_counts'] /
                                                                 restaurants_and_reviews['text']
                                                                 .str.split().apply(len)) * 100
...
labels = [0, 1, 2, 3]
restaurants_and_reviews['green_rating'] = pd.cut(restaurants_and_reviews['env_terms_percent_of_overall_words'],
...
restaurants_and_reviews[['name', 'text', 'green_rating']].to_csv(
    '../data/small_env_term_results.csv', index=False)
